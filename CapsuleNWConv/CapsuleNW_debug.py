...
class CapsuleNetwork(torch.nn.Module):
    ...
    def forward(self, input, labels = None):
        ...
        if labels is not None:
            ...
            u_hat = u_hat.sum(dim=1, keepdim=True)
            u_hat = self.squash(u_hat, dim=-2)
            v_k, _ = self.safeNorm(u_hat, dim=-2, keepdim=True)
            margin_loss = self.loss_fn(torch.squeeze(v_k), T)
